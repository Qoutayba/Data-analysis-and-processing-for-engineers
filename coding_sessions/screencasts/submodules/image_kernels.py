#!/usr/bin/env python3
import numpy as np
import general_functions as generate
...
def disc( n_x,n_y=None,kind='ones'):
    """
    Return an unpadded circle kernel of shape==(n_y,n_x)
...
def pacman(n, kind='ones'):
    """
    Return an unpadded diamond kernel of shape==(n,n)
...
def cross( n ):
    """
    define a cross kernel which is an X in the image of values 1
    its width is defined by the size, equalling n//20 + 2
    Parameters:
    -----------
    n:      int
            size of the cross kernel
    Returns:
    --------
    kernel: numpy 2d-array
            array of kernel in tight image representation
    """
    width = n//20 + 2
    width = width + (width % 2)
    cross = generate.band_diagonal( n, width)
    cross += np.flip( cross, axis=1)
